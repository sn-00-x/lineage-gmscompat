...
for branch in ${BRANCH_NAME//,/ }; do
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    for path in "vendor/cm" "vendor/lineage" "frameworks/base" "packages/apps/PermissionController" "build/soong" "bionic" "libcore"; do
      if [ -d "$path" ]; then
        cd "$path" || exit
        git reset -q --hard
        git clean -q -fd
    ...
    if [ "$additional_patches" = true ] ; then
      echo ">> [$(date)] Applying low ram and gmscompat patches"

      # low ram patches
      cd "$SRC_DIR/$branch_dir/build/soong"
      for i in /root/low_ram_patches/*.patch ; do
        patch --quiet --force -p1 < $i
      done

      # gmscompat patches
      for path in "bionic" "frameworks/base" "libcore"; do
        cd "$SRC_DIR/$branch_dir/$path"
        for i in /root/gmscompat_patches/$path/*.patch ; do
          patch --quiet --force -p1 < $i
        done
      done
